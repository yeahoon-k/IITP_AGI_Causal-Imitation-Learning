...
if __name__ == '__main__':
    ...
    # # Seq_mSBD_test
    # G = CD({'X1', 'Z1', 'X2', 'Z2', 'Y1', 'Y'},
    #        [('Z1', 'X1'), ('Z1', 'Y1'), ('X1', 'Y1'),
    #                        ('Z2', 'X2'), ('Z2', 'Y'), ('X2', 'Y'),
    #                        ('Z1', 'X2'), ('Z1', 'Y'), ('Z1', 'Y1'),
    #                        ('X1', 'X2'), ('X1', 'Y'), ('X1', 'Z2'),
    #                        ('Y1', 'Z2'), ('Y1', 'X2'), ('Y1', 'Y')])
    # #order = ['Z1','X1', 'Y1', 'Z2', 'X2', 'Y2']
    # policy = seq_pi_bd(G, {'X1', 'X2'}, {'Y'}, {'Z1', 'X1', 'Y1', 'Z2', 'X2', 'Y'})
    # print(policy)

    # mSBD t=2
    # G =CD('Z1->X1, Z1->Z2, Z1->Y1, Z1->Y, Z1->X2, X1->Y1, X1->X2, X1->Y, X1->Z2, Y1->X2, Y1->Z2, Y1->Y, Z2->Y, Z2->X2, X2->Y')
    # policy = seq_pi_bd(G, {'X1', 'X2'}, {'Y'}, ['Z1', 'X1', 'Y1', 'Z2', 'X2', 'Y'])
    # print(policy)

    # mSBD t=2
    # G =CD('Z1->X1, Z1->Z2, Z1->Y1, Z1->Y, Z1->X2, X1->Y1, X1->X2, X1->Y, X1->Z2, Y1->X2, Y1->Z2, Y1->Y, Z2->Y, Z2->X2, X2->Y')
    # policy = seq_pi_bd(G, {'X1', 'X2'}, {'Y'}, ['Z1', 'X1', 'X2', 'Y1', 'Z2', 'Y'])
    # print(policy)

    # Kumor et al. table 1 - (2), (3)
    # G =CD('X1->X2->Y, X1<->Z, Z->Y')
    # policy = seq_pi_bd(G, {'X1', 'X2'}, {'Y'}, ['Z', 'X1', 'X2', 'Y'])
    # print(policy)
    # policy2 = seq_pi_bd(G, {'X1', 'X2'}, {'Y'}, ['X1', 'Z', 'X2', 'Y'])
    # print(policy2)


    #### self-ai 2024 experiments
    # From Daniel Kumor's algorithm
    # G = CD('X1<->Z1, X1->X2->Y, Z1->Y, Y->X3, X3<->Z2, X3->X4->Y2, Z2->Y2')
    # policy_Y1 = seq_pi_bd(G, {'X1', 'X2'}, 'Y',
    #                    ['X1', 'Z1', 'X2', 'Y', 'Z2', 'X3', 'X4', 'Y2'])
    #
    # G = CD('X1<->Z1, X1->X2->Y1, Z1->Y1, Y1->X3, X3<->Z2, X3->X4->Y, Z2->Y')
    # policy_Y2 = seq_pi_bd(G, {'X3', 'X4'}, 'Y',
    #                    ['X1', 'Z1', 'X2', 'Y1', 'X3', 'Z2', 'X4', 'Y'])
    #
    # print(f"policy Y1 : {policy_Y1}, policy Y2 : {policy_Y2}")


    # # Kumor et al. table 1 - (4)
    # G = CD('Z->X2->Y, Y<->Z, Z->X1, X1->Y')
    # policy = seq_pi_bd(G, {'X1', 'X2'}, {'Y'}, ['X1', 'Z', 'X2', 'Y'])
    # print(policy)

    # test_imi_mSBD2 = CD('X1<->Z1->Z2->Z3->Y, X1->X2->X3->Y')
    # policy = seq_pi_bd(test_imi_mSBD2, {'X1', 'X2', 'X3'}, {'Y'},
    #                    ['X1', 'Z1', 'X2', 'Z2', 'X3', 'Z3', 'Y'])
    # print(policy)
    # print(policy)
